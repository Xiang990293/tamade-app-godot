...
func _ready() -> void:
	position = get_window().size/2 - Vector2i($Control.size/2)
	pass
...
func pick_random_coordinate() -> void:
	...
	var poly_size = $tamade_button_shape.polygon[2]
	target_pos = Vector2(randf_range(0, win_size[0]-poly_size[0]),randf_range(0, win_size[1]-poly_size[1]))
	print("pick_random_coordinate() = ",target_pos)
